...
async def reader_lh_test(dut):
    ...
    for _ in range(100):
        ...
        mem_data = random.randint(0x80000000, 0xFFFFFFFF) | 0xAEAE0000
        dut.mem_data.value = mem_data
...
async def reader_lb_test(dut):
    ...
    await Timer(1, unit="ns")
    for _ in range(100):
        mem_data = random.randint(0, 0x00007FFF) | 0xAE000000
        dut.mem_data.value = mem_data
        await Timer(1, unit="ns")
        assert dut.wb_data.value == (mem_data & 0x0000FF00) >> 8
        assert dut.valid.value == 1

        mem_data = random.randint(0x00008000, 0x0000FFFF) | 0xAE000000
        dut.mem_data.value = mem_data
    ...
    await Timer(1, unit="ns")
    for _ in range(100):
        mem_data = random.randint(0, 0x0000007F) | 0xAE000000
        dut.mem_data.value = mem_data
        await Timer(1, unit="ns")
        assert dut.wb_data.value == (mem_data & 0x000000FF)
        assert dut.valid.value == 1

        mem_data = random.randint(0x00000080, 0x000000FF) | 0xAE000000
        dut.mem_data.value = mem_data
